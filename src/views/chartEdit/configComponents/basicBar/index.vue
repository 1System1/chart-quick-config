<template>
    <div class="slots">
        ...
        <el-collapse v-if="!loading" v-model="activeName" accordion :collapse-transition="false">
            ...
            <!-- X轴  -->
            <chart-x-axis :customConfig="customConfig" @handleChangeXAxis="handleChangeXAxis" />
            <!-- Y轴  -->
            <chart-y-axis :customConfig="customConfig" @handleChangeYAxis="handleChangeYAxis" />
        </el-collapse>
    </div>
</template>
<script>
    ...
    export default defineComponent({
        name: 'BasicBar',
        components: { chartDataset, chartTheme, chartTitle, chartLegend, chartTooltip, chartKeyMap, chartXAxis, chartYAxis },
        props: {
            data: {
                type: String,
                default: () => '[]',
        ...
        setup(props, { emit }) {
            ...
            const handleChangeXAxis = (xAxis) => {
                Object.assign(state.customConfig, xAxis);
                changeCustomConfig();
            };
            // y轴
            const handleChangeYAxis = (yAxis) => {
                Object.assign(state.customConfig, yAxis);
                changeCustomConfig();
            };
